...
func (u *User) GenCaptcha(c *gin.Context) {
	...
	if err != nil {
		...
		ginx.BombErr(errors2.CodeInternalServerError, errors2.GetErrorMessage(errors2.CodeInternalServerError))
	}
...
func (u *User) SendSms(c *gin.Context) {
	...
	if utils.VerifyMobilePhoneFormat(req.Phone) {
		...
		ginx.BombErr(errors2.CodeInvalidParams, "invalid phone number")
	}
	...
	if !ok {
		...
		ginx.BombErr(errors2.CodeForbidden, "wrong captcha answer")
	}
...
func (u *User) VerifyAndUpdatePhone(c *gin.Context) {
	...
	if !ok {
		...
		ginx.BombErr(errors2.CodeForbidden, "verify code is not correct")
	}
	if isExpire {
		log.Errorf("sms code is expired, code: %s, traceID: %s", req.VerifyCode, traceID)
		ginx.BombErr(errors2.CodeTokenExpire, "sms code is expired")
	}
